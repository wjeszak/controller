...
void Enc28j60::SPI_Wyslij(uint8_t bajt)
{
	SPDR = bajt;
	spi_czekaj();
}

uint8_t Enc28j60::SPI_Odbierz()
{
	uint8_t bajt = SPDR;
	return bajt;
}
...
void Enc28j60::RejWyczyscBity(uint8_t rejestr, uint8_t bity)
{
...
void Enc28j60::RejUstawBity(uint8_t rejestr, uint8_t bity)
{
	// UWAGA!! Trzeba wczesniej ustawic bank !!
	// Jesli nie operuje sie na rejestrach zmapowanych
	uint8_t adres = rejestr & ADR_MASKA;
...
uint8_t Enc28j60::RejCzytaj(uint8_t rejestr)
{
	...
	if(adres < 0x1A)
	{
		...
		uint8_t bank = (rejestr & BANK_MASKA) >> 5;
		if(bank != enc28j60_Bank)
		{
			// Kasowanie bitow BSEL0, BSEL1 poprzez wartosc 0x03
			RejWyczyscBity(ENC_REG_ECON1, (0x03 << ENC_BIT_BSEL0));

			if(bank)
			{
				RejUstawBity(ENC_REG_ECON1, bank << ENC_BIT_BSEL0);
			}
			enc28j60_Bank = bank;
		}
	}
...
void Enc28j60::RejZapisz(uint8_t rejestr, uint8_t wart_rejestru)
{
	uint8_t adres = rejestr & ADR_MASKA;
		// Jesli czytamy rejestry niewspolne
		if(adres < 0x1A)
...
*/
void Enc28j60::ZapiszPhy(uint8_t rejPhy, uint16_t wartosc_rej)
{
	RejZapisz(ENC_REG_MIREGADR, rejPhy);
	RejZapisz(ENC_REG_MIWRL, LO8(wartosc_rej));
	RejZapisz(ENC_REG_MIWRH, HI8(wartosc_rej));
	_delay_us(10);
	while(RejCzytaj(ENC_REG_MISTAT) & (1 << ENC_BIT_BUSY));
}

void Enc28j60::CzytajBuf(uint8_t *buf, uint16_t dl)
{
	ENC28J60_CS_AKTYWNY;
...
void Enc28j60::ZapiszBuf(uint8_t *buf, uint16_t dl)
{
	ENC28J60_CS_AKTYWNY;
...
//	ENC_DEBUG("enc_send: %i bytes\n", len);
	...
	while( ms--)
	{
		if(!(RejCzytaj(ENC_REG_ECON1) & (1 << ENC_BIT_TXRTS))) break;
		_delay_ms(1);
	}
	#ifdef FULL_DUPLEX
	// Reset jesli bit nadal ustawiony, errata #12: Reset TX Logic
	if(RejCzytaj(ENC_REG_ECON1) & (1 << ENC_BIT_TXRTS))
	{
	#else
	if(1)
	{
		...
		RejUstawBity(ENC_REG_ECON1, (1 << ENC_BIT_TXRST));
		RejWyczyscBity(ENC_REG_ECON1, (1 << ENC_BIT_TXRST));
	}
	// Ustaw wskaznik zapisu poczatek
	RejZapisz(ENC_REG_EWRPTL, LO8(ENC_TX_BUFFER_START));
	RejZapisz(ENC_REG_EWRPTH, HI8(ENC_TX_BUFFER_START));

	// Ustaw wskaznik zapisu koniec
	RejZapisz(ENC_REG_ETXNDL, LO8(ENC_TX_BUFFER_START + dl));
	RejZapisz(ENC_REG_ETXNDH, HI8(ENC_TX_BUFFER_START + dl));
	...
	// Czyszczenie flagi
	RejWyczyscBity(ENC_REG_EIR, (1 << ENC_BIT_TXIF));
...
uint16_t Enc28j60::ReceivePacket(uint16_t rozmiar_buf, uint8_t *buf)
{
	...
	RejZapisz(ENC_REG_ERDPTL, LO8(WskNastepnyPakiet));
	RejZapisz(ENC_REG_ERDPTH, HI8(WskNastepnyPakiet));

	// Naglowek
	CzytajBuf(rx_naglowek, sizeof(rx_naglowek));
	...
	if( dl > rozmiar_buf ) dl = rozmiar_buf;
		...
		{
			RejZapisz(ENC_REG_ERXRDPTL, LO8(ENC_RX_BUFFER_END));
			RejZapisz(ENC_REG_ERXRDPTH, HI8(ENC_RX_BUFFER_END));
		}
		else
		{
			RejZapisz(ENC_REG_ERXRDPTL, LO8(WskNastepnyPakiet - 1));
			RejZapisz(ENC_REG_ERXRDPTH, HI8(WskNastepnyPakiet - 1));
		}

		// trigger a decrement of the rx packet counter
		// this will clear PKTIF if EPKTCNT reaches 0
		RejUstawBity(ENC_REG_ECON2, (1 << ENC_BIT_PKTDEC));
...
void Enc28j60::Init()
{
	SPI_Init();
	Reset();
	while(!(RejCzytaj(ENC_REG_ESTAT) & (1 << ENC_BIT_CLKRDY)));
	_delay_ms(20);


	RejZapisz(ENC_REG_ETXSTL, LO8(ENC_TX_BUFFER_START));
	RejZapisz(ENC_REG_ETXSTH, HI8(ENC_TX_BUFFER_START));
	RejZapisz(ENC_REG_ETXNDL, LO8(ENC_TX_BUFFER_END));
	RejZapisz(ENC_REG_ETXNDH, HI8(ENC_TX_BUFFER_END));

		// rx buffer
	RejZapisz(ENC_REG_ERXSTL, LO8(ENC_RX_BUFFER_START));
	RejZapisz(ENC_REG_ERXSTH, HI8(ENC_RX_BUFFER_START));
	RejZapisz(ENC_REG_ERXNDL, LO8(ENC_RX_BUFFER_END));
	RejZapisz(ENC_REG_ERXNDH, HI8(ENC_RX_BUFFER_END));

		// push mac out of reset
	RejZapisz(ENC_REG_MACON2, 0x00);

		// mac receive enable, rx and tx pause control frames enable
	RejZapisz(ENC_REG_MACON1, (1<<ENC_BIT_MARXEN) | (1<<ENC_BIT_RXPAUS) | (1<<ENC_BIT_TXPAUS));

		#ifdef FULL_DUPLEX
		// mac auto padding of small packets, add crc, frame length check, full duplex
	RejZapisz(ENC_REG_MACON3, (1<<ENC_BIT_PADCFG0) | (1<<ENC_BIT_TXCRCEN)
	...
	RejZapisz(ENC_REG_ECOCON, 0x00);
		...
		ZapiszPhy(ENC_REG_PHCON1, HI8(ENC_REG_PHCON1_VALUE)| LO8(ENC_REG_PHCON1_VALUE));
		#endif

		#ifndef FULL_DUPLEX
		// in half duplex do not loop back transmitted data
		#define ENC_REG_PHCON2_VALUE (0x0000 | (1 << ENC_BIT_HDLDIS))
		ZapiszPhy(ENC_REG_PHCON2, HI8(ENC_REG_PHCON2_VALUE)| LO8(ENC_REG_PHCON2_VALUE));
		#endif
