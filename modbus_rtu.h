...
#define MODBUS_RTU_RES_HOLDING_BYTE_COUNT_HI	 	2
#define MODBUS_RTU_RES_HOLDING_BYTE_COUNT_LO 		3
#define MODBUS_RTU_RES_HOLDING_REG_VAL_HI 			4
#define MODBUS_RTU_RES_HOLDING_REG_VAL_LO 			5
...
public:
	ModbusRTU();
	void PrepareFrameHolding(uint8_t address, uint8_t function);
	void PrepareFrameSingle(uint8_t address, uint8_t function, uint16_t value);
	void PrepareFrameCRC();
	void Poll();
